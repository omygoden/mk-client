...
function handleGlobalShortcuts(e) {
  ...
  if ((e.ctrlKey || e.metaKey) && e.key === 'f') {
    if (currentViewMode === 'edit' || currentViewMode === 'split' || currentViewMode === 'live') {
      e.preventDefault();
      showFindBar();
    }
  }
...
function showFindBar() {
  ...
  const sel = markdownTextarea.value.substring(
...
function scrollToMatch(index) {
  ...
  markdownTextarea.focus();
  markdownTextarea.setSelectionRange(m.start, m.end);

  // Scroll to the selection — estimate line position
...
async function openFile() {
  if (isModified) {
    const confirmLeave = confirm('You have unsaved changes. Do you want to continue?');
    if (!confirmLeave) return;
  }
...
function loadFile(filePath, content, fileName) {
  ...
  if (currentViewMode === 'live') {
    renderLiveEditMode();
    // Focus the contentEditable area so the cursor appears and editing works
    requestAnimationFrame(() => {
      previewContent.focus();
      // Place cursor at the beginning
      if (previewContent.firstChild) {
        const range = document.createRange();
        const textNode = previewContent.firstChild.firstChild || previewContent.firstChild;
